...
static GBUF(m_str);
static GBUF(r_str);
static FormatPrintLen str_len = {0, 0, 0};
...
static const FormatOption *find_fopt(const FormatOption *fopts, const char *key)
{
    const FormatOption *fo;
...
static const char *str_val(const char *key, const FormatOption *fopts, char *buf)
{
    const FormatOption *fo;
    const Option *opt;
    const char *val = NULL;
...
static int int_val(const char *key, const FormatOption *fopts, char *buf)
{
    const FormatOption *fo;
    int val = -1;
...
static int format_eval_cond(struct expr* expr, const FormatOption *fopts)
{
    if (!expr)
        return -1;
...
static void format_parse(int str_width, const char *format, const FormatOption *fopts, int f_size);

static void format_parse_if(int str_width, const char *format, const FormatOption *fopts, int *s)
{
...
static void format_read(int str_width, const char *format, const FormatOption *fopts)
{
    gbuf_clear(&l_str);
    gbuf_clear(&m_str);
    gbuf_clear(&r_str);
...
FormatPrintLen format_print(struct gbuf *buf, int str_width, const char *format, const FormatOption *fopts)
{
...
static int format_valid_sub(const char *format, const FormatOption *fopts, int f_size);

static int format_valid_if(const char *format, const FormatOption *fopts, int *s)
{
