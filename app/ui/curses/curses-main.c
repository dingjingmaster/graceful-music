...
int                         gUIInitialized = 0;
UICursesInputMode           gInputMode = NORMAL_MODE;
int                         gCurView = TREE_VIEW;
int                         gPrevView = -1;
Searchable*                 gSearchable;
char*                       gLibFilename = NULL;
char*                       gLibExtFilename = NULL;
char*                       gPlayQueueFilename = NULL;
char*                       gPlayQueueExtFilename = NULL;
...
static FormatOption track_fopts[NR_TFS + 1] = {
...
static inline void fopt_set_str(FormatOption *fopt, const char *str)
{
...
static inline void fopt_set_int(FormatOption *fopt, int value, int empty)
{
...
static inline void fopt_set_double(FormatOption *fopt, double value, int empty)
{
...
static inline void fopt_set_time(FormatOption *fopt, int value, int empty)
{
...
const FormatOption *get_global_fopts(void)
{
    if (player_info.ti)
...
static void update_sorted_window(void)
{
    ...
    update_editable_window(&lib_editable, "Library", gLibFilename);
}
...
static void post_update(void)
{
    ...
    if (gInputMode == COMMAND_MODE || gInputMode == SEARCH_MODE) {
...
void set_view(int view)
{
    ...
    switch (gCurView) {
        case TREE_VIEW:
            gSearchable = tree_searchable;
            break;
        case SORTED_VIEW:
            gSearchable = lib_editable.shared->searchable;
            break;
        case PLAYLIST_VIEW:
            gSearchable = pl_get_searchable();
            break;
        case QUEUE_VIEW:
            gSearchable = pq_editable.shared->searchable;
            break;
        case BROWSER_VIEW:
            gSearchable = browser_searchable;
            break;
        case FILTERS_VIEW:
            gSearchable = filters_searchable;
            break;
        case HELP_VIEW:
...
static void handle_ch(uchar ch)
{
    ...
    if (gInputMode == NORMAL_MODE) {
        normal_mode_ch(ch);
    } else if (gInputMode == COMMAND_MODE) {
        command_mode_ch(ch);
        update_commandline();
    } else if (gInputMode == SEARCH_MODE) {
        search_mode_ch(ch);
        update_commandline();
    }
}
