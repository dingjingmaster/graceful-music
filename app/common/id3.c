...
#define id3_debug(...) DEBUG(__VA_ARGS__)

const char * const gId3KeyNames[NUM_ID3_KEYS] = {
    "artist",
    "album",
    "title",
    "date",
...
static struct {
    ...
    { "TDOR", ID3_ORIGINAL_DATE }, // original release date
    { "TSOP", ID3_ARTIST_SORT },
    { "TSOA", ID3_ALBUM_SORT },
    ...
    { "TPOS", ID3_DISC },
    { "TRCK", ID3_TRACK },
    { "TPE2", ID3_ALBUM_ARTIST },
    { "TSO2", ID3_ALBUM_ARTIST_SORT },
    { "XSOP", ID3_ARTIST_SORT }, // obsolete
    { "XSOA", ID3_ALBUM_SORT }, // obsolete
    { "TCMP", ID3_COMPILATION },
    { "TORY", ID3_ORIGINAL_DATE },
    { "TCOM", ID3_COMPOSER },
    { "TPE3", ID3_CONDUCTOR },
    { "TEXT", ID3_LYRICIST },
    { "TPE4", ID3_REMIXER },
...
static void add_v2(Id3Tag* id3, enum Id3Key key, char *value)
{
    free(id3->v2[key]);
    id3->v2[key] = value;
    id3->hasV2 = 1;
...
static void decode_normal(Id3Tag* id3, const char *buf, int len, int encoding, enum Id3Key key)
{
    ...
    if (key == ID3_GENRE) {
        ...
        free(out);
        out = tmp;
    } else if (key == ID3_DATE || key == ID3_ORIGINAL_DATE) {
    ...
    } else if (key == ID3_ALBUM_ARTIST) {
        /*
         * This must be TPE2 frame; ignore it if ID3_ALBUN_ARTIST is
         * already present
         */
        if (id3->v2[key]) {
            free(out);
...
static void decode_txxx(Id3Tag* id3, const char *buf, int len, int encoding)
{
    const char ql_prefix[] = "QuodLibet::";
    enum Id3Key key = NUM_ID3_KEYS;
    int size;
    char *out, *out_mem;
...
static void decode_comment(Id3Tag* id3, const char *buf, int len, int encoding)
{
    int slen;
    char *out;
 ...
 */
static void decode_rva2(Id3Tag* id3, const char *buf, int len)
{
...
static void decode_ufid(Id3Tag* id3, const char *buf, int len)
{
    char *ufid;
...
static void v2_add_frame(Id3Tag* id3, struct v2_frame_header *fh, const char *buf)
{
    int encoding;
    int len;
    int idx;
...
static int v2_read(Id3Tag* id3, int fd, const struct v2_header *header)
{
    char *buf;
    int rc, buf_size;
    int frame_start, i;
...
{
    const Id3Tag t = { .hasV1 = 0, .hasV2 = 0 };
    *id3 = t;
}
