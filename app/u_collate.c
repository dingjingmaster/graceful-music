...
#include "convert.h"
#include "global.h"
#include "curses-main.h" /* using_utf8, charset */
...
int u_strcoll(const char *str1, const char *str2)
{
	...
	if (gUsingUtf8) {
		...
		convert(str1, -1, &str1_locale, -1, gCharset, "UTF-8");
		convert(str2, -1, &str2_locale, -1, gCharset, "UTF-8");
...
char *u_strcoll_key(const char *str)
{
	...
	if (!result) {
		...
		convert(str, -1, &str_locale, -1, gCharset, "UTF-8");
